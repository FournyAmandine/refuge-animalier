...
<article {!! $attributes->merge(['class'=>'relative hover:scale-105 hover:duration-200 hover:transition-all flex flex-wrap justify-between items-center border-2 rounded-lg p-2.5 [box-shadow:var(--shadow-xl)] mb-5 min-[1920px]:max-w-[64rem]']) !!}>
    <button class="absolute inset-0" wire:click="toggleModal('show', {{$id}})" title="{{ __('volunteer_message.view_message') }}"></button>
    ...
    <p class="text-sm">
        {{ __('volunteer_message.days_ago', ['days' => $day]) }}
    </p>

    <x-admin.button
        wire:click="toggleReadMessage({{$id}})"
        href="mailto:{{ $email }}"
        title="{{ __('volunteer_message.send_email_to', ['name' => $name]) }}"
        label="{{ $label }}"
