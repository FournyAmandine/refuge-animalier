                ...
                <span class="paginate relative z-0 inline-flex rtl:flex-row-reverse rounded-md">
                    ...
                    @if ($paginator->onFirstPage())
                        <span class="paginate" aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <span class="paginate relative inline-flex items-center px-2 py-2 text-sm font-medium text-orange-600 cursor-default hover:bg-orange-100/30 rounded-l-md leading-5" aria-hidden="true">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-orange-600/70  leading-5 hover:bg-orange-100/30 focus:z-10 focus:outline-none focus:border-b-orange-600 rounded-lg active:bg-orange-100 active:text-orange-600 transition ease-in-out duration-150 dark:bg-orange-50  dark:text-orange-600/70 dark:hover:text-orange-600 dark:active:bg-orange-100 dark:focus:border-orange-800" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
